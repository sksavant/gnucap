/* $Id: b3soild.c,v 3.0 02/5/20 Pin Su Release $  */
/*
$Log:   b3soild.c,v $
 ...
 *
*/
static char rcsid[] = "$Id: b3soild.c,v 3.0 02/5/20 Pin Su Release $";
...
/* v3.0 */
		   ...
		   (*(ckt->CKTrhs + here->B3SOIgNode) -= (ceqgate + ceqqg) + Igtoteq); /* v3.0 */
		   (*(ckt->CKTrhs + here->B3SOIdNodePrime) += (ceqbd - cdreq - ceqqd) + Idtoteq); /* v3.0 */
...
/* v3.0 */
                   ...
                   *(here->B3SOIGePtr) += gcgeb;
                   ...
                   (*(here->B3SOIGbPtr) -= -gigb + gcggb + gcgdb + gcgsb + gcgeb - gIgtotb); /* v3.0 */
                   (*(here->B3SOIDPbPtr) -= (-gddpb - Gmbs + gcdgb + gcddb + gcdeb + gcdsb) + gIdtotb); /* v3.0 */
		   ...
		   (*(here->B3SOIGgPtr) += gigg + gcggb + ckt->CKTgmin + gIgtotg); /* v3.0 */
		   (*(here->B3SOIGdpPtr) += gigd + gcgdb - ckt->CKTgmin + gIgtotd); /* v3.0 */
		   (*(here->B3SOIGspPtr) += gcgsb + gigs + gIgtots); /* v3.0 */

		   (*(here->B3SOIDPgPtr) += (Gm + gcdgb) + gddpg - ckt->CKTgmin - gIdtotg); /* v3.0 */
		   (*(here->B3SOIDPdpPtr) += (here->B3SOIdrainConductance
					 + here->B3SOIgds + gddpdp
					 + RevSum + gcddb) + ckt->CKTgmin - gIdtotd); /* v3.0 */
		   (*(here->B3SOIDPspPtr) -= (-gddpsp + here->B3SOIgds + FwdSum - gcdsb) + gIdtots); /* v3.0 */
					 
		   (*(here->B3SOIDPdPtr) -= here->B3SOIdrainConductance);

		   (*(here->B3SOISPgPtr) += gcsgb - Gm + gsspg - gIstotg); /* v3.0 */
		   (*(here->B3SOISPdpPtr) -= (here->B3SOIgds - gsspdp + RevSum - gcsdb + gIstotd)); /* v3.0 */
