/* $Id: b3soild.c,v 2.2.2 01/2/15 Pin Su Release $  */
/*
$Log:   b3soild.c,v $
 ...
 *
*/
static char rcsid[] = "$Id: b3soild.c,v 2.2.2 01/2/15 Pin Su Release $";
...
/* v2.2: calculate gate-tunneling-to-body current */
                 ...
                 here->B3SOIgigs = -(dIgb_dVg + dIgb_dVd + dIgb_dVb);
		  ...
		  {   Gm = -here->B3SOIgm;
		   ...
		   Gmin = ckt->CKTgmin * 1e-6;
                   ...
                   *(here->B3SOIGePtr) += gcgeb;
                   ...
                   (*(here->B3SOIGbPtr) -= -gigb + gcggb + gcgdb + gcgsb + gcgeb);
                   ...
                   (*(here->B3SOIBspPtr) += gcbsb + gbbsp - Gmin 
                                         - gigs);
                   (*(here->B3SOIBbPtr) += -gigb + gbbb - gcbgb - gcbdb - gcbsb - gcbeb + Gmin) ;
		   (*(here->B3SOIEePtr) += gceeb);

		   (*(here->B3SOIGgPtr) += gigg + gcggb + ckt->CKTgmin);
		   (*(here->B3SOIGdpPtr) += gigd + gcgdb - ckt->CKTgmin);
