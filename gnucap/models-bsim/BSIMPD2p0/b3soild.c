/* $Id: b3soild.c,v 2.0 99/2/15 Pin Su Release $  */
/*
$Log:   b3soild.c,v $
 ...
 *
*/
static char rcsid[] = "$Id: b3soild.c,v 2.0 99/2/15 Pin Su Release $";
...
/*  Calculate temperature dependent values for self-heating effect  */
		  ...
		  if (selfheat) {
		      ...
		      if (model->B3SOInsub > 0) {
                         T0 = log(model->B3SOInpeak / model->B3SOInsub);
		         vfbb = -model->B3SOItype * Vtm * T0;
                         dvfbb_dT = -model->B3SOItype * KboQ * T0;
                      } 
		      else {
                         T0 = log(-model->B3SOInpeak * model->B3SOInsub / ni / ni);
		         vfbb = -model->B3SOItype * Vtm * T0;
...
/*		      phi = 2.0 * Vtm * log(pParam->B3SOInpeak / ni);  */
		      ...
		      here->B3SOIni = ni;
                      ...
                      dT7_dT = model->B3SOIxbjt * dT4_dT / model->B3SOIndiode;
                      DEXP(T7, T0, dT0_dT7);
                      ...
                      if (model->B3SOIxbjt == model->B3SOIxdif) {
                         ...
                         dT7_dT = model->B3SOIxdif * dT4_dT / model->B3SOIndiode;
                         DEXP(T7, T1, dT1_dT7);
                         dT1_dT = dT1_dT7 * dT7_dT;
                      }

                      T7 = model->B3SOIxrec * T4 / model->B3SOInrecf0;
                      dT7_dT = model->B3SOIxrec * dT4_dT / model->B3SOInrecf0;
                      DEXP(T7, T2, dT2_dT7);
                      dT2_dT = dT2_dT7 * dT7_dT;
 
                      /* high level injection */
                      Ahli = model->B3SOIahli * T0;
                      dAhli_dT = model->B3SOIahli * dT0_dT;
 
                      jbjt = model->B3SOIisbjt * T0;
                      jdif = model->B3SOIisdif * T1;
                      jrec = model->B3SOIisrec * T2;
                      djbjt_dT = model->B3SOIisbjt * dT0_dT;
                      djdif_dT = model->B3SOIisdif * dT1_dT;
                      djrec_dT = model->B3SOIisrec * dT2_dT;
 
                      T7 = model->B3SOIxtun * T3;
                      dT7_dT = model->B3SOIxtun * T8;
                      DEXP(T7, T0, dT0_dT7);
                      dT0_dT = dT0_dT7 * dT7_dT;
                      jtun = model->B3SOIistun * T0;
                      djtun_dT = model->B3SOIistun * dT0_dT;
...
/* Calculate Abulk */
                  if (pParam->B3SOIa0 == 0.0) {
                     ...
                     T10 = pParam->B3SOIphi + model->B3SOIketas;
                     T13 = (Vbsh * T11) / T10;
                     ...
                     T10 = 0.5 * pParam->B3SOIk1eff 
                         / sqrt(pParam->B3SOIphi + model->B3SOIketas);
                     T1 = T10 * T14;
...
/* This includes all dependencies from Vgsteff, Vbseff */
		  ...
		  Gm = Gm0 * dVgsteff_dVg / here->B3SOInseg;
		  Gmb = (Gm0 * dVgsteff_dVb + Gmb0 * dVbseff_dVb) / here->B3SOInseg;
		  Gds = (Gm0 * dVgsteff_dVd + Gds0) / here->B3SOInseg;
		  if (selfheat)
		     GmT = (Gm0 * dVgsteff_dT + GmT0) / here->B3SOInseg;
		  else GmT = 0.0;

/*  calculate GIDL current  */
		  T0 = 3 * model->B3SOItox;
		  /* For drain side */
		  T1 = (Vds - Vgs_eff - model->B3SOIngidl) / T0;
		  if ((model->B3SOIagidl <= 0.0) || (model->B3SOIbgidl <= 0.0) ||
		      (T1 <= 0.0))
		  {   Idgidl = Gdgidld = Gdgidlg = 0.0;
		     ...
		     T2 = model->B3SOIbgidl / T1;
		     if (T2 < EXP_THRESHOLD)
		     {
			Idgidl = pParam->B3SOIwdiod * model->B3SOIagidl * T1 * exp(-T2);
			T3 = Idgidl / T1 * (T2 + 1);
			Gdgidld = T3 * dT1_dVd;
			Gdgidlg = T3 * dT1_dVg;
		     } else
		     {
			T3 = pParam->B3SOIwdiod * model->B3SOIagidl * MIN_EXP;
			Idgidl = T3 * T1 ;
			Gdgidld  = T3 * dT1_dVd;
			Gdgidlg  = T3 * dT1_dVg;
		     } 
		  ...
		  /* For source side */
		  T1 = (- Vgs_eff - model->B3SOIngidl) / T0;
		  if ((model->B3SOIagidl <= 0.0) || (model->B3SOIbgidl <= 0.0) 
                        || (T1 <= 0.0))
		  {   Isgidl = Gsgidlg = 0;
		  }
		  else
		  {
		     ...
		     if (T2 < EXP_THRESHOLD)
		     {
			Isgidl = pParam->B3SOIwdios * model->B3SOIagidl * T1 * exp(-T2);
			T3 = Isgidl / T1 * (T2 + 1);
			Gsgidlg = T3 * dT1_dVg;
		     } else
		     {
			T3 = pParam->B3SOIwdios * model->B3SOIagidl * MIN_EXP;
			Isgidl = T3 * T1 ;
			Gsgidlg = T3 * dT1_dVg;
		     } 
		  }
...
/* calculate diode and BJT current */
                  WsTsi = pParam->B3SOIwdios * model->B3SOItsi;
                  WdTsi = pParam->B3SOIwdiod * model->B3SOItsi;

                  NVtm1 = Vtm * model->B3SOIndiode;
                  if (selfheat)
                     dNVtm1_dT = model->B3SOIndiode * dVtm_dT;
                  else 
                     dNVtm1_dT = 0;
		  ...
		  /* Ibs2:recombination/trap-assisted tunneling current */
                  NVtmf = 0.026 * model->B3SOInrecf0 
                        * (1 + model->B3SOIntrecf * (TempRatio - 1));
                  NVtmr = 0.026 * model->B3SOInrecr0 
                        * (1 + model->B3SOIntrecr * (TempRatio - 1));
                  if (selfheat) {
                     dNVtmf_dT = model->B3SOInrecf0 * 0.026 
                               * model->B3SOIntrecf * dTempRatio_dT;
                     dNVtmr_dT = model->B3SOInrecr0 * 0.026 
                  ...
                  if (jrec == 0) {
                     ...
                     /* reverse bias */
                     if ((model->B3SOIvrec0 - Vbs) < 1e-3) {
                        T11 = -1.0;
                        dT11_dVb = dT11_dT = 0;
                     }
                     else {
                        T1 = 1 / (model->B3SOIvrec0 - Vbs);
                        T0 = -Vbs / NVtmr * model->B3SOIvrec0 * T1;
                        dT0_dVb = -model->B3SOIvrec0 / NVtmr * (T1 + Vbs * T1 * T1) ;
                        if (selfheat)
                     ...
                     if ((model->B3SOIvrec0 - Vbd) < 1e-3) {
                        T11 = -1.0;
                        dT11_dVb = dT11_dT = 0;
                     }
                     else {
                        T1 = 1 / (model->B3SOIvrec0 - Vbd);
                        T0 = -Vbd / NVtmr * model->B3SOIvrec0 * T1;
                        dT0_dVb = -model->B3SOIvrec0 / NVtmr * (T1 + Vbd * T1 * T1) ;
                        if (selfheat)
                           dT0_dT = -T0 / NVtmr * dNVtmr_dT;
                        else
                           dT0_dT = 0;
		  ...
		  {  
		     ...
		     here->B3SOIic = Ic = Gcd = Gcb = GcT = 0.0;
                  ...
                  /* Ibs4/Ibd4 : tunneling */
                  NVtm2 = 0.026 * model->B3SOIntun;
                  if (jtun == 0)
                  {  Ibs4 = Ibd4 = dIbs4_dVb = dIbs4_dT = dIbd4_dVb = dIbd4_dVd = dIbd4_dT = 0;
                  } else
                  {
                     if ((model->B3SOIvtun0 - Vbs) < 1e-3)
                        Ibs4 = dIbs4_dVb = dIbs4_dT = 0;
                     else {
                        T1 = 1 / (model->B3SOIvtun0 - Vbs);
                        T0 = -Vbs / NVtm2 * model->B3SOIvtun0 * T1;
                        dT0_dVb = -model->B3SOIvtun0 / NVtm2 * (T1 + Vbs * T1 * T1) ;
                     ...
                     if ((model->B3SOIvtun0 - Vbd) < 1e-3)
                        Ibd4 = dIbd4_dVb = dIbd4_dT = 0;
                     else {
                        T1 = 1 / (model->B3SOIvtun0 - Vbd);
                        T0 = -Vbd / NVtm2 * model->B3SOIvtun0 * T1;
                        dT0_dVb = -model->B3SOIvtun0 / NVtm2 * (T1 + Vbd * T1 * T1) ;
...
/* calculate substrate current Iii */
                  ...
                  if (pParam->B3SOIalpha0 <= 0.0) {
                     ...
                     Vdsatii0 = model->B3SOIvdsatii0 * (1 + model->B3SOItii * (TempRatio-1.0))
                        - model->B3SOIlii / Leff;
                     if (selfheat) 
                        dVdsatii0_dT = model->B3SOIvdsatii0 * model->B3SOItii * dTempRatio_dT;
                     else 
			dVdsatii0_dT = 0;

                     /* Calculate VgsStep */
                     T0 = model->B3SOIesatii * Leff;
                     T1 = model->B3SOIsii0 * T0 / (1.0 + T0);

                     T0 = 1 / (1 + model->B3SOIsii1 * Vgsteff);
                     if (selfheat)
                        dT0_dT = - model->B3SOIsii1 * T0 * T0 *dVgsteff_dT;
                     else
                        dT0_dT = 0;
                     T3 = T0 + model->B3SOIsii2;
                     T4 = Vgst * model->B3SOIsii1 * T0 * T0;
                     T2 = Vgst * T3;
                     ...
                     T3 = 1 / (1 + model->B3SOIsiid * Vds);
                     dT3_dVd = - model->B3SOIsiid * T3 * T3;
                     ...
                     T0 = model->B3SOIbeta2 + model->B3SOIbeta1 * Vdiff
                        + model->B3SOIbeta0 * Vdiff * Vdiff;
                     if (T0 < 1e-5)
                     {
                        T0 = 1e-5;
                     ...
                     {
                        T1 = model->B3SOIbeta1 + 2 * model->B3SOIbeta0 * Vdiff;
                     ...
                     if ((T0 < Vdiff / EXP_THRESHOLD) && (Vdiff > 0.0)) {
                        Ratio = model->B3SOIalpha0 * MAX_EXP;
                        dRatio_dVg = dRatio_dVb = dRatio_dVd = dRatio_dT = 0.0;
                     }
                     else if ((T0 < -Vdiff / EXP_THRESHOLD) && (Vdiff < 0.0)) {
                        Ratio = model->B3SOIalpha0 * MIN_EXP;
                        dRatio_dVg = dRatio_dVb = dRatio_dVd = dRatio_dT = 0.0;
                     }
                     else {
                        Ratio = model->B3SOIalpha0 * exp(Vdiff / T0);
                        T1 = Ratio / T0 / T0;
                     ...
                     T0 = Ids + model->B3SOIfbjtii * Ic;
                     here->B3SOIiii = Iii = Ratio * T0;
                     Giig = Ratio * Gm + T0 * dRatio_dVg;
                     Giib = Ratio * (Gmb + model->B3SOIfbjtii * Gcb) 
                          + T0 * dRatio_dVb;
                     Giid = Ratio * (Gds + model->B3SOIfbjtii * Gcd) 
                          + T0 * dRatio_dVd;

                     if (selfheat)
                        GiiT = Ratio * (GmT + model->B3SOIfbjtii * GcT) 
                             + T0 * dRatio_dT;
                     else
                        GiiT = 0.0;
	...
	/* Current through body resistor */
		  ...
		  here->B3SOIcjd = Ibd - Iii - Idgidl 
				 ...
				 +  here->B3SOIgjdg * Vgs + here->B3SOIgjdT * delTemp);
	...
	/*  Begin CV (charge) model  */
		  ...
		  else 
		  {
		       ...
		       {   ExpVgst *= ExpVgst;
                           ExpVgst *= exp( -(model->B3SOIdelvt / (n * Vtm)));
			   ...
			   dVgsteff_dVg = T0 * dVgs_eff_dVg;
                           dVgsteff_dVb = T1 * dVbseff_dVb;
                           if (selfheat)
			      dVgsteff_dT = -T0 * (dVth_dT + (Vgst - model->B3SOIdelvt) / Temp) 
                                          + Vgsteff / Temp;
		       ...
		       {   
                          Vfb = Vth - phi - pParam->B3SOIk1eff * sqrtPhis + model->B3SOIdelvt;
			  ...
			  {   /* 50/50 Charge partition model */
			     ...
			     Csd1 = - 0.5 * (Cgd1 + Cbd1);
		          ...
		          /* Backgate charge */
		          CboxWL = model->B3SOIkb1 * model->B3SOIfbody * Cbox  
                                 * (pParam->B3SOIweffCV / here->B3SOInseg
			  ...
			  here->B3SOIcdsb = (Cgg + Cgd + Cgb 
                      ...
                      else if (model->B3SOIcapMod == 3)
                      {
                         ...
                         if (selfheat) {
                            ...
                            Vfbzb = pParam->B3SOIvfbzb + model->B3SOIdelvt;
                            dVfbzb_dT = 0;
                         }
                         ...
                         {   Tcen = pParam->B3SOIldeb * MIN_EXP;
                             dTcen_dVg = dTcen_dVb = dTcen_dT = 0.0;
                         }
                         else
                         {   Tcen = pParam->B3SOIldeb * MAX_EXP;
                             dTcen_dVg = dTcen_dVb = dTcen_dT = 0.0;
                         }

                         LINK = 1.0e-3 * model->B3SOItox;
			 ...
			 * pParam->B3SOIwdiosCV * model->B3SOItsi / 1e-7;
			 ...
			 dT3_dVb = exp(-MJSWG * log(arg));
		      ...
		      /* T10 is vse without type conversion */
		         ...
		         {
		            if (T10 < model->B3SOIvsdfb)
		            {  here->B3SOIqse = pParam->B3SOIcsbox * (T10 - model->B3SOIvsdfb);
			       here->B3SOIgcse = pParam->B3SOIcsbox;
		            }
		            else if (T10 < pParam->B3SOIsdt1)
		            {  T0 = T10 - model->B3SOIvsdfb;
			       T1 = T0 * T0;
			       here->B3SOIqse = T0 * (pParam->B3SOIcsbox - 
                                                pParam->B3SOIst2 / 3 * T1) ;
			       here->B3SOIgcse = pParam->B3SOIcsbox - pParam->B3SOIst2 * T1;
		            }
		            else if (T10 < model->B3SOIvsdth)
		            {  T0 = T10 - model->B3SOIvsdth;
			       T1 = T0 * T0;
			       here->B3SOIqse = pParam->B3SOIcsmin * T10 + pParam->B3SOIst4 + 
                                                pParam->B3SOIst3 / 3 * T0 * T1;
   			    here->B3SOIgcse = pParam->B3SOIcsmin + pParam->B3SOIst3 * T1;
		            }
		            else 
		            {  here->B3SOIqse = pParam->B3SOIcsmin * T10 + pParam->B3SOIst4;
			       here->B3SOIgcse = pParam->B3SOIcsmin;
		            }
		         } else
		         {
		            if (T10 < model->B3SOIvsdth)
		            {  here->B3SOIqse = pParam->B3SOIcsmin * (T10 - model->B3SOIvsdth);
			       here->B3SOIgcse = pParam->B3SOIcsmin;
		            }
		            else if (T10 < pParam->B3SOIsdt1)
		            {  T0 = T10 - model->B3SOIvsdth;
   			       T1 = T0 * T0;
   			       here->B3SOIqse = T0 * (pParam->B3SOIcsmin - pParam->B3SOIst2 / 3 * T1) ;
			       here->B3SOIgcse = pParam->B3SOIcsmin - pParam->B3SOIst2 * T1;
		            }
		            else if (T10 < model->B3SOIvsdfb)
		            {  T0 = T10 - model->B3SOIvsdfb;
			       T1 = T0 * T0;
			       here->B3SOIqse = pParam->B3SOIcsbox * T10 + pParam->B3SOIst4 + 
                                                pParam->B3SOIst3 / 3 * T0 * T1;
			       here->B3SOIgcse = pParam->B3SOIcsbox + pParam->B3SOIst3 * T1;
		            }
		            else 
		            {  here->B3SOIqse = pParam->B3SOIcsbox * T10 + pParam->B3SOIst4;
			       here->B3SOIgcse = pParam->B3SOIcsbox;
		         ...
		         if ( ((model->B3SOInsub > 0) && (model->B3SOItype > 0)) ||
		              ((model->B3SOInsub < 0) && (model->B3SOItype < 0)) )
		         {
		            if (T11 < model->B3SOIvsdfb)
		            {  here->B3SOIqde = pParam->B3SOIcdbox * (T11 - model->B3SOIvsdfb);
			       here->B3SOIgcde = pParam->B3SOIcdbox;
		            }
		            else if (T11 < pParam->B3SOIsdt1)
		            {  T0 = T11 - model->B3SOIvsdfb;
   			       T1 = T0 * T0;
   			       here->B3SOIqde = T0 * (pParam->B3SOIcdbox - pParam->B3SOIdt2 / 3 * T1) ;
			       here->B3SOIgcde = pParam->B3SOIcdbox - pParam->B3SOIdt2 * T1;
		            }
		            else if (T11 < model->B3SOIvsdth)
		            {  T0 = T11 - model->B3SOIvsdth;
			       T1 = T0 * T0;
			       here->B3SOIqde = pParam->B3SOIcdmin * T11 + pParam->B3SOIdt4 + 
                                                pParam->B3SOIdt3 / 3 * T0 * T1;
			       here->B3SOIgcde = pParam->B3SOIcdmin + pParam->B3SOIdt3 * T1;
		            }
		            else 
		            {  here->B3SOIqde = pParam->B3SOIcdmin * T11 + pParam->B3SOIdt4;
			       here->B3SOIgcde = pParam->B3SOIcdmin;
		            }
		         } else
		         {
		            if (T11 < model->B3SOIvsdth)
		            {  here->B3SOIqde = pParam->B3SOIcdmin * (T11 - model->B3SOIvsdth);
			       here->B3SOIgcde = pParam->B3SOIcdmin;
		            }
		            else if (T11 < pParam->B3SOIsdt1)
		            {  T0 = T11 - model->B3SOIvsdth;
   			       T1 = T0 * T0;
   			       here->B3SOIqde = T0 * (pParam->B3SOIcdmin - pParam->B3SOIdt2 / 3 * T1) ;
			       here->B3SOIgcde = pParam->B3SOIcdmin - pParam->B3SOIdt2 * T1;
		            }
		            else if (T11 < model->B3SOIvsdfb)
		            {  T0 = T11 - model->B3SOIvsdfb;
			       T1 = T0 * T0;
			       here->B3SOIqde = pParam->B3SOIcdbox * T11 + pParam->B3SOIdt4 + 
                                                pParam->B3SOIdt3 / 3 * T0 * T1;
			       here->B3SOIgcde = pParam->B3SOIcdbox + pParam->B3SOIdt3 * T1;
		            }
		            else 
		            {  here->B3SOIqde = pParam->B3SOIcdbox * T11 + pParam->B3SOIdt4;
			       here->B3SOIgcde = pParam->B3SOIcdbox;
                         ...
                         here->B3SOIqse = pParam->B3SOIcsbox * T10;
                         here->B3SOIgcse = pParam->B3SOIcsbox;
                         here->B3SOIqde = pParam->B3SOIcdbox * T11;
                         here->B3SOIgcde = pParam->B3SOIcdbox;
                      }
	...
	line755:
		  ...
		  {   gcdgb = (here->B3SOIcdgb - cgdo) * ag0;
		      ...
		      gcggb = (here->B3SOIcggb + cgdo + cgso + model->B3SOIcgeo) * ag0;
		      gcgdb = (here->B3SOIcgdb - cgdo) * ag0;
		      gcgsb = (here->B3SOIcgsb - cgso) * ag0;
		      gcgeb = (- model->B3SOIcgeo) * ag0;
		  ...
		  {   gcsgb = (here->B3SOIcdgb - cgso) * ag0;
		      ...
		      gcesb = (- here->B3SOIgcse) * ag0;
		      gcedb = (- here->B3SOIgcde) * ag0;
		      gceeb = (here->B3SOIceeb + model->B3SOIcgeo +
		  ...
		  {   Gm = -here->B3SOIgm;
		   ...
		   {   
		       ...
		       ceqqe = -ceqqe;
                   ...
                   (*(ckt->CKTrhs + here->B3SOIbNode) -=(ceqbody+ceqqb));
		   ...
		   Gmin = ckt->CKTgmin * 1e-6;
                   (*(here->B3SOIEbPtr) -= gcegb + gcedb + gcesb + gceeb);
                   (*(here->B3SOIGbPtr) -= gcggb + gcgdb + gcgsb + gcgeb);
                   (*(here->B3SOIDPbPtr) -= (-gddpb - Gmbs + gcdgb + gcddb + gcdeb + gcdsb));
                   (*(here->B3SOISPbPtr) -= (-gsspb + Gmbs + gcsgb + gcsdb + gcseb + gcssb));
                   (*(here->B3SOIBePtr) += gcbeb);
                   (*(here->B3SOIBgPtr) += gcbgb + gbbg);
                   (*(here->B3SOIBdpPtr) += gcbdb + gbbdp );
                   (*(here->B3SOIBspPtr) += gcbsb + gbbsp - Gmin);
                   (*(here->B3SOIBbPtr) += gbbb - gcbgb - gcbdb - gcbsb - gcbeb + Gmin) ;
