 ...
 *
*/
static char rcsid[] = "$Id: b3soild.c, DD2.1 99/9/27 Pin Su Release $";
...
/*		      phi = 2.0 * Vtm * log(pParam->B3SOInpeak / ni);  */
		      ...
		      jtun = model->B3SOIistun * T0;
                      djtun_dT = model->B3SOIistun * T7 * pow(TempRatio, T7 - 1.0) * T8;
...
/* calculate substrate current Iii */
	          T2 = pParam->B3SOIalpha1 + pParam->B3SOIalpha0 / Leff;
		  if ((T2 <= 0.0) || (pParam->B3SOIbeta0 <= 0.0))
		  {   Giig = Giib = Giid = Giie = GiiT = 0.0;
		      here->B3SOIiii = Iii = 0.0;
		  }
		  else 
		  {   
		      T5 = pParam->B3SOIbeta0;
...
/*  calculate GIDL current  */
		  ...
		  {   Idgidl = Gdgidld = Gdgidlg = 0.0;
		     ...
		     if (T2 < EXP_THRESHOLD)
		     {
			Idgidl = pParam->B3SOIweff * pParam->B3SOIagidl * T1 * exp(-T2);
			T3 = Idgidl / T1 * (T2 + 1);
			Gdgidld = T3 * dT1_dVd;
			Gdgidlg = T3 * dT1_dVg;
		     } else
		     {
			T3 = pParam->B3SOIweff * pParam->B3SOIagidl * MIN_EXP;
			Idgidl = T3 * T1 ;
			Gdgidld  = T3 * dT1_dVd;
			Gdgidlg  = T3 * dT1_dVg;
		     } 
		  ...
		  else
		  {
		     ...
		     if (T2 < EXP_THRESHOLD)
		     {
			Isgidl = pParam->B3SOIweff * pParam->B3SOIagidl * T1 * exp(-T2);
			T3 = Isgidl / T1 * (T2 + 1);
			Gsgidlg = T3 * dT1_dVg;
		     } else
		     {
...
/* calculate diode and BJT current */
		  ...
		  WTsi = pParam->B3SOIweff * model->B3SOItsi;
		  NVtm1 = Vtm * pParam->B3SOIndiode;
		  NVtm2 = Vtm * pParam->B3SOIntun;
		  ...
		  {  
		     ...
		     T0 = Leff - pParam->B3SOIkbjt1 * Vds;
   		     T1 = T0 / pParam->B3SOIedl;
                     dT1_dVd = - pParam->B3SOIkbjt1 / pParam->B3SOIedl;
		     ...
		     here->B3SOIic = Ic = Ibjt - Ibs3 + Ibd3;
	...
	/* Current through body resistor */
		  ...
		  if ((here->B3SOIbodyMod == 0) || (here->B3SOIbodyMod == 2)) 
			...
			if (selfheat)  GbpT = T0 * dVbp_dT;
                     ...
                     {  T0 = 1.0 / pParam->B3SOIrbody;
                           if (Vbp >= 0.0)
                           {
                              T1 = sqrt(Vcs);
                              T3 = T1 * T0;
                              T5 = 1.0 + here->B3SOIrbodyext * T3;
                              T6 = T3 / T5;
                              T2 = 0.5 * T0 / T1;
                              T7 = T2 / (T5 * T5);
                              Ibp = Vbp * T6;
		...
		/*  Whoa, again these derivatives are convoluted, but correct  */
		  ...
		  here->B3SOIgbpgs = Gbpgs;
		  here->B3SOIgbpds = Gbpds;
		  here->B3SOIgbpes = Gbpes;
		  ...
		  here->B3SOIcbodcon = Ibp - (Gbpbs * Vbs + Gbpgs * Vgs 
	...
	/*  Begin CV (charge) model  */
		  ...
		  else 
		  {
			 ...
			 {   
			    ...
			    T2 = -model->B3SOIcboxt * pParam->B3SOIweffCV
			       * pParam->B3SOIleffCV;
			    T3 = T2 * 0.5 * (1 - Xc);
	...
	finished: /* returning Values to Calling Routine */
		  ...
		  if (ChargeComputationNeeded)
		      ...
		      cjsbs = model->B3SOIunitLengthGateSidewallJctCap
				 *  pParam->B3SOIweff * model->B3SOItsi / 1e-7;
		      if (Vbs < 0.0) 
		      {  arg = 1.0 - Vbs / PhiBSWG;
			 if (MJSWG == 0.5)
			 ...
			 dT3_dVb = 1 + MJSWG * Vbs / PhiBSWG;
                      ...
                      qjs = cjsbs * T3 + model->B3SOItt * Ibs1;
                      gcjsbs = cjsbs * dT3_dVb + model->B3SOItt * dIbs1_dVb;
                      if (selfheat)  gcjsT = model->B3SOItt * dIbs1_dT;
                      else  gcjsT = 0.0;
			 ...
			 dT3_dVb = 1 + MJSWG * Vbd / PhiBSWG;
		      ...
		      dT3_dVd = - dT3_dVb;
                      ...
                      qjd = cjsbs * T3 + model->B3SOItt * Ibd1;
                      gcjdbs = cjsbs * dT3_dVb + model->B3SOItt * dIbd1_dVb;
                      gcjdds = cjsbs * dT3_dVd + model->B3SOItt * dIbd1_dVd;
                      if (selfheat) gcjdT = model->B3SOItt * dIbd1_dT;
                      else  gcjdT = 0.0;
...
/*  Here NaN will be detected in any conductance or equivalent current.  Note
    that nandetect is initialized within the "if" statements  */
                      ...
                      if (nandetect = isnan (*(here->B3SOIGbPtr)))
                      { strcpy (nanmessage, "GbPtr"); }
                      else if (nandetect = isnan (*(here->B3SOIEbPtr)))
                      { strcpy (nanmessage, "EbPtr"); }
                      else if (nandetect = isnan (*(here->B3SOIDPbPtr)))
                      { strcpy (nanmessage, "DPbPtr"); }
                      else if (nandetect = isnan (*(here->B3SOISPbPtr)))
                      { strcpy (nanmessage, "SPbPtr"); }
                      else if (nandetect = isnan (*(here->B3SOIBbPtr)))
                      { strcpy (nanmessage, "BbPtr"); }
                      else if (nandetect = isnan (*(here->B3SOIBgPtr)))
                      { strcpy (nanmessage, "BgPtr"); }
                      else if (nandetect = isnan (*(here->B3SOIBePtr)))
                      { strcpy (nanmessage, "BePtr"); }
                      else if (nandetect = isnan (*(here->B3SOIBdpPtr)))
                      { strcpy (nanmessage, "BdpPtr"); }
                      else if (nandetect = isnan (*(here->B3SOIBspPtr)))
...
/*  I am trying to debug the convergence problems here by printing out
  ...
  fprintf(fpdebug, "Eg = %.5e;\t Edp = %.5e;\t Esp = %.5e;\t Eb = %.5e;\n",
		   *(here->B3SOIEgPtr), *(here->B3SOIEdpPtr), *(here->B3SOIEspPtr),
		   *(here->B3SOIEbPtr));
