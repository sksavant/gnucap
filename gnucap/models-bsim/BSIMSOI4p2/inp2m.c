...
/*  See if 5th token after device specification is a model name  */
	     ...
	     INPgetMod(ckt,model,&thismodel,tab);  /*  get pointer to the model  */
             if (thismodel != NULL) {
                if ((thismodel->INPmodType != INPtypelook("B3SOI")) 
                   ...
                   LITERR("only level 9, 10 B3/B4SOI can have 7 nodes")
                   return;
                }
                else {  /*  if looking at B3/B4SOI model, allocate the 7th node  */
	        ...
	        if ((thismodel->INPmodType != INPtypelook("B3SOI"))
                     && (thismodel->INPmodType != INPtypelook("B4SOI")) ) {
	                /*  if model is not variable node B3SOI model, error!  */
                   LITERR("only level 9, 10 B3/B4SOI can have 6 nodes")
                   return;
	        }
	        else {  /*  if looking at B3/B4SOI model, allocate the 6th node  */
                   INPtermInsert(ckt,&nname5,tab,&node5);
                   INPtermInsert(ckt,&nname6,tab,&node6);
                   model = nname7;
	        }
	        ...
	        else {  /*  if looking at B3/B4SOI model, allocate the 5th node  */
                   INPtermInsert(ckt,&nname5,tab,&node5);
    ...
    if(thismodel != NULL) {
        if (thismodel->INPmodType != INPtypelook("Mos1")
                ...
                && thismodel->INPmodType != INPtypelook("BSIM5")
