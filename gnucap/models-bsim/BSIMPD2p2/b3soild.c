/* $Id: b3soild.c,v 2.2 00/3/1 Pin Su Release $  */
/*
$Log:   b3soild.c,v $
 ...
 *
*/
static char rcsid[] = "$Id: b3soild.c,v 2.2 00/3/1 Pin Su Release $";
...
/* Calculate Abulk */
                  if (pParam->B3SOIa0 == 0.0) {
                     ...
                     T10 = pParam->B3SOIphi + pParam->B3SOIketas;
                     T13 = (Vbsh * T11) / T10;
                     ...
                     T10 = 0.5 * pParam->B3SOIk1eff 
                         / sqrt(pParam->B3SOIphi + pParam->B3SOIketas);
                     T1 = T10 * T14;
...
/* v2.2: calculate gate-tunneling-to-body current */
                  ...
                  if (model->B3SOIigMod >= 1) {
                     ...
                     if (pParam->B3SOIk1eff == 0.0) {
                        ...
                        Voxdepinv = Vgs_eff - (T1*T1 + Vbseff) - Vfb;
                        dVoxdepinv_dVg = dVgs_eff_dVg - (2.0*T1*dT1_dVg);
                        dVoxdepinv_dVd = -(2.0*T1*dT1_dVd);
                        dVoxdepinv_dVb = -(2.0*T1*dT1_dVb + dVbseff_dVb);
                        if (selfheat)
                     ...
                     T0 = (Vox - model->B3SOIebg)/model->B3SOInevb/Vtm;
                     if (selfheat)
                        dT0_dT = (dVox_dT/Vtm - (Vox - model->B3SOIebg)/Vtm/Vtm*dVtm_dT)
                               /model->B3SOInevb;
                     ...
                     T0 = (-Vgb+(Vfb))/model->B3SOInecb/Vtm;
                     if (selfheat)
                     ...
                     Vaux = model->B3SOInecb * Vtm * log(1 + T1);
                     dVaux_dVg = -T2 / (1 + T1);
                     dVaux_dVd = 0;
                     dVaux_dVb = -dVaux_dVg;
                     if (selfheat)
                        dVaux_dT = model->B3SOInecb * (dVtm_dT* log(1+T1)
                                 + Vtm * dT1_dT / (1+T1));
                     ...
                     if (selfheat) dIgb_dT = dIgb1_dT + dIgb2_dT;                   
...
/* calculate substrate current Iii */
	...
	/* Current through body resistor */
		  ...
		  if (selfheat) here->B3SOIgbT = GiiT - GjsT - GjdT + dIgb_dT;
	...
	/*  Begin CV (charge) model  */
		  ...
		  else 
		  {
			   ...
			   dVgsteff_dVg = T0 * dVgs_eff_dVg;
                           dVgsteff_dVb = T1 * dVbseff_dVb;
                           if (selfheat)
			      dVgsteff_dT = -T0 * (dVth_dT + (Vgst - pParam->B3SOIdelvt) / Temp) 
                                          + Vgsteff / Temp;
	...
	line900:
		  ...
		  {   Gm = -here->B3SOIgm;
		   ...
		   (*(ckt->CKTrhs + here->B3SOIgNode) -= (ceqgate + ceqqg));
		   ...
		   Gmin = ckt->CKTgmin * 1e-6;
                   ...
                   *(here->B3SOIDPePtr) += gcdeb;
                   *(here->B3SOISPePtr) += gcseb;
                   *(here->B3SOIEgPtr) += gcegb;
                   *(here->B3SOIGePtr) += gcgeb;

                   (*(here->B3SOIEbPtr) -= gcegb + gcedb + gcesb + gceeb);
                   (*(here->B3SOIGbPtr) -= -dIgb_dVb + gcggb + gcgdb + gcgsb + gcgeb);
                   (*(here->B3SOIDPbPtr) -= (-gddpb - Gmbs + gcdgb + gcddb + gcdeb + gcdsb));

                   (*(here->B3SOISPbPtr) -= (-gsspb + Gmbs + gcsgb + gcsdb + gcseb + gcssb)) + Gmin;         


                   (*(here->B3SOIBePtr) += gcbeb);
                   (*(here->B3SOIBgPtr) += -dIgb_dVg + gcbgb + gbbg);
                   (*(here->B3SOIBdpPtr) += -dIgb_dVd + gcbdb + gbbdp );
                   (*(here->B3SOIBspPtr) += gcbsb + gbbsp - Gmin);
                   (*(here->B3SOIBbPtr) += -dIgb_dVb + gbbb - gcbgb - gcbdb - gcbsb - gcbeb + Gmin) ;
		   (*(here->B3SOIEePtr) += gceeb);

		   (*(here->B3SOIGgPtr) += dIgb_dVg + gcggb + ckt->CKTgmin);
		   (*(here->B3SOIGdpPtr) += dIgb_dVd + gcgdb - ckt->CKTgmin);
		   ...
		   if (selfheat) 
		      ...
		      (*(here->B3SOIBtempPtr) += gbbT + gcbT - dIgb_dT);
                      (*(here->B3SOIEtempPtr) += gceT);
                      (*(here->B3SOIGtempPtr) += gcgT + dIgb_dT);
