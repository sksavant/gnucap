/* $Id: b3soild.c,v 3.1.1 03/4/20 Pin Su Release $  */
/*
$Log:   b3soild.c,v $
 ...
 *
*/
static char rcsid[] = "$Id: b3soild.c,v 3.1.1 03/4/20 Pin Su Release $";
...
/* B/S built-in potential lowering calculation */
                  ...
                  else /* soiMod = 1 or 2: adding FD module on top of BSIMPD */
                  {
                     ...
                     if (model->B3SOIsoiMod == 2) /* v3.1 ideal FD: Vbsitf is pinned at Vbs0 */
                     {
...
/* v3.0 */
                  ...
                  if ((model->B3SOIigbMod!= 0) && (model->B3SOIsoiMod != 2)) 
...
/* v3.1 */
                          if (model->B3SOIsoiMod == 2) /* ideal FD */
                          {           
...
/* v3.1 wanh end*/
	...
	line860:
		  ...
		  /* v3.1 wanh added gcegmb and gcdgmb in ceqqb and ceqqd for RF */
                  ceqqb = cqbody - gcbgb * vgb + gcbdb * vbd + gcbsb * vbs
                          - gcbeb * veb - gcbT * delTemp -gcegmb *  vgme;
                  ceqqd = cqdrn - gcdgb * vgb + gcddb * vbd + gcdsb * vbs
                          - gcdeb * veb - gcdT * delTemp -gcdgmb * vgme;
		  /* v3.1 wanh added gcegmb and gcdgmb in ceqqb and ceqqd for RF end */
		  ...
		  /* v3.1 wanh added for RF */
                  if (here->B3SOIrgateMod == 3)
                        ceqqgmid = *(ckt->CKTstate0 + here->B3SOIcqgmid)
                         + gcgmdb * vbd + gcgmsb * vbs - gcgmgmb * vgme;
                  else
                        ceqqgmid = 0.0;
		  /* v3.1 wanh added for RF end */
...
/* v3.1 */
		   ...
		   (*(ckt->CKTrhs + here->B3SOIdNodePrime) += (ceqbd - cdreq
							    - ceqqd) + Idtoteq); /* v3.0 */
		   /* v3.1 wanh added ceqqgmid for RF */
		   (*(ckt->CKTrhs + here->B3SOIsNodePrime) += (cdreq + ceqbs
							    + ceqqg
		   ...
		   /* v3.1 wanh added for RF */
                   if (here->B3SOIrgateMod == 2)
                       (*(ckt->CKTrhs + here->B3SOIgNodeExt) -= ceqgcrg);
                   else if (here->B3SOIrgateMod == 3)
                       (*(ckt->CKTrhs + here->B3SOIgNodeMid) -= ceqqgmid + ceqgcrg);
		   /* v3.1 wanh added for RF end */
		   ...
		   /* v3.1 wanh added for RF */
		      ...
		      *(here->B3SOIGEgePtr) += geltd;
               	      ...
               	      *(here->B3SOIGMgmPtr) += geltd + gcrg + gcgmgmb;

               	      *(here->B3SOIGMdpPtr) += gcrgd + gcgmdb;
               	      *(here->B3SOIGMgPtr) += gcrgg;
               	      *(here->B3SOIGMspPtr) += gcrgs + gcgmsb;
               	      *(here->B3SOIGMePtr) += gcgmeb;
		      if (model->B3SOIsoiMod !=2)
               	         *(here->B3SOIGMbPtr) += gcrgb;

               	      *(here->B3SOIDPgmPtr) += gcdgmb;
               	      *(here->B3SOIGgmPtr) -= gcrg;
                      *(here->B3SOISPgmPtr) += gcsgmb;
               	      *(here->B3SOIEgmPtr) += gcegmb;
           	   }
		   /* v3.1 wanh added for RF end*/
...
/* v3.1 */
		      ...
		      if ((here->B3SOIrgateMod == 0) || (here->B3SOIrgateMod == 1))
                         (*(here->B3SOIGbPtr) -= -gigb + gcggb + gcgdb + gcgsb
			  		      + gcgeb - gIgtotb);
		      else /* v3.1 wanh added for rgateMod = 2 or 3 */ 
                         *(here->B3SOIGbPtr) += gigb + gcgbb +gIgtotb - gcrgb;


                      (*(here->B3SOIDPbPtr) -= (-gddpb - Gmbs + gcdgb + gcddb
					     + gcdeb + gcdsb) + gIdtotb); /* v3.0 */
                      (*(here->B3SOISPbPtr) -= (-gsspb + Gmbs + gcsgb + gcsdb
				             + gcseb + gcssb) 
                                             + Gmin + gIstotb); /* v3.0 */        

                      (*(here->B3SOIBePtr) += gbbe + gcbeb); /* v3.0 */
                      (*(here->B3SOIBgPtr) += -gigg + gcbgb + gbbg);
                      (*(here->B3SOIBdpPtr) += -gigd + gcbdb + gbbdp );
                      (*(here->B3SOIBspPtr) += gcbsb + gbbsp - Gmin 
                                            - gigs);
                      (*(here->B3SOIBbPtr) += -gigb + gbbb - gcbgb - gcbdb
					    - gcbsb - gcbeb + Gmin) ;
                   }
/* v3.1 */
		   ...
		   {
		     (*(here->B3SOIGgPtr) += gigg + gcggb + Gmin
					   + gIgtotg); 
		     (*(here->B3SOIGdpPtr) += gigd + gcgdb - Gmin
					    + gIgtotd); 
		     (*(here->B3SOIGspPtr) += gcgsb + gigs + gIgtots);
		   }
		   ...
		   { 
                     ...
                     *(here->B3SOIGdpPtr) += gigd + gcgdb - Gmin
					   + gIgtotd - gcrgd;
                     *(here->B3SOIGspPtr) += gcgsb + gigs + gIgtots - gcrgs;
		   }

		   
		   (*(here->B3SOIDPgPtr) += (Gm + gcdgb) + gddpg - Gmin
					  - gIdtotg); /* v3.0 */ 
		   (*(here->B3SOIDPdpPtr) += (here->B3SOIdrainConductance
					   + here->B3SOIgds + gddpdp
					   + RevSum + gcddb) + Gmin
					   - gIdtotd); /* v3.0 */
		   (*(here->B3SOIDPspPtr) -= (-gddpsp + here->B3SOIgds + FwdSum
					   - gcdsb) + gIdtots); /* v3.0 */
					 
		   (*(here->B3SOIDPdPtr) -= here->B3SOIdrainConductance);

		   (*(here->B3SOISPgPtr) += gcsgb - Gm + gsspg - gIstotg); /* v3.0 */
		   (*(here->B3SOISPdpPtr) -= (here->B3SOIgds - gsspdp + RevSum
					   - gcsdb + gIstotd)); /* v3.0 */
		   ...
		   if (selfheat) 
		      ...
		      (*(here->B3SOIBtempPtr) += gbbT + gcbT - gigT);
                      (*(here->B3SOIEtempPtr) += gceT);
                      (*(here->B3SOIGtempPtr) += gcgT + gigT);
