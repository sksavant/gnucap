/*$Id: e_homebrew.cc,v 1.2 2010-07-09 12:14:23 felix Exp $ -*- C++ -*-
 * vim:ts=8:sw=2:et
...
/*--------------------------------------------------------------------------*/
HOMEBREW::HOMEBREW(const HOMEBREW& p)
  :COMPONENT(p),
   _loaditer(0),
   _m0(),
   _m1(),
...
/*--------------------------------------------------------------------------*/
bool HOMEBREW::skip_dev_type(CS& cmd)
{
  return cmd.umatch(dev_type() + ' ');
}
/*--------------------------------------------------------------------------*/
void HOMEBREW::precalc_last()
{
...
/*--------------------------------------------------------------------------*/
void HOMEBREW::tr_iwant_matrix_passive()
{
  ...
  if (_n[OUT1].m_() == INVALID_NODE) {
    std::cerr << "HOMEBREW::tr_iwant_matrix_passive " << long_label() << "\n";
    exit(4);
  }
...
/*--------------------------------------------------------------------------*/
void HOMEBREW::tr_iwant_matrix_active()
{
  std::cerr << "HOMEBREW::tr_iwant_matrix_active " << long_label() << "\n";
...
/*--------------------------------------------------------------------------*/
void HOMEBREW::tr_iwant_matrix_extended()
{
  assert(is_device());
  assert(!subckt());
...
/*--------------------------------------------------------------------------*/
void HOMEBREW::ac_iwant_matrix_passive()
{
  trace2(long_label().c_str(), _n[OUT1].m_(), _n[OUT2].m_());
  _sim->_acx.iwant(_n[OUT1].m_(),_n[OUT2].m_());
}
/*--------------------------------------------------------------------------*/
void HOMEBREW::ac_iwant_matrix_active()
{
  //_sim->_acx.iwant(_n[OUT1].m_(),_n[OUT2].m_());
...
/*--------------------------------------------------------------------------*/
void HOMEBREW::ac_iwant_matrix_extended()
{
  assert(is_device());
  assert(!subckt());
  assert(ext_nodes() + int_nodes() == matrix_nodes());
...
/*--------------------------------------------------------------------------*/
hp_float_t HOMEBREW::tr_amps()const
{
	//return 44;
...
/*--------------------------------------------------------------------------*/
void HOMEBREW::tr_save_amps(int n){

//	std::cerr << "HOMEBREW::tr_save_amps " << short_label() << "\n";
	...
	if(_amps!=0) {
		...
		std::cerr << "HOMEBREW::tr_save_amps: not first. diff= " << _tr_amps_diff_cur << "\n";
	} else {
	...
	std::cerr << short_label() << " have " << tramps << "\n";
	_amps_new[ n ]= tramps;
...
/*--------------------------------------------------------------------------*/
COMPLEX HOMEBREW::ac_amps()const
{
  assert(!is_source());
  return (ac_involts() * _acg + ac_outvolts() * (double)_loss0);
}
/*--------------------------------------------------------------------------*/
XPROBE HOMEBREW::ac_probe_ext(const std::string& x)const
{
...
/*--------------------------------------------------------------------------*/
double HOMEBREW::tr_review_trunc_error(const FPOLY1* q)
{
...
/*--------------------------------------------------------------------------*/
double HOMEBREW::tr_review_check_and_convert(double timestep)
{
  double time_future;
  if (timestep == NEVER) {
    time_future = NEVER;
