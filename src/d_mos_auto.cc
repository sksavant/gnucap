/* $Id: d_mos.model,v 1.3 2010-06-29 10:50:14 felix Exp $ -*- C++ -*-
vim:ts=8:sw=2:et:
...
void COMMON_BUILT_IN_MOS::expand(const COMPONENT* d)
{
  ...
  COMMON_BUILT_IN_RCD* bu = new COMMON_BUILT_IN_RCD;
  bu->set_modelname(modelname());
  bu->attach(model());
  attach_common(bu, &_bu);
  ...
  if (_sim->is_first_expand()) {
    ...
    if (!(_n[n_u_eff].n_())) {
      if (something) {
        ...
        _n[n_u_eff].new_model_node("." + long_label() + ".u_eff", this);
      }
    }else{
      if (something) {
        assert(_n[n_u_eff] == _n[n_b]);
      }else{
        //_n[n_u_eff].new_model_node("u_eff." + long_label(), this);
      }
    }
    ...
    }
    {
      ...
      }
      {
        ...
        _BTI->set_parameters("BTI", this, NULL, 1.0, 0, NULL, 2, nodes);
      }
    }
    {
      if (!_Ids) {
...
double DEV_BUILT_IN_MOS::tr_probe_num(const std::string& x)const
{
  ...
  const SDP_BUILT_IN_MOS_BASE* s = prechecked_cast<const SDP_BUILT_IN_MOS_BASE*>(c->sdp());
  assert(s);
  const ADP_BUILT_IN_MOS_BASE* a = prechecked_cast<const ADP_BUILT_IN_MOS_BASE*>(adp()); a=a;
...
double DEV_BUILT_IN_MOS::tt_probe_num(const std::string& x)const
{
  ...
  const SDP_BUILT_IN_MOS_BASE* s = prechecked_cast<const SDP_BUILT_IN_MOS_BASE*>(c->sdp());
  assert(s);
  const ADP_BUILT_IN_MOS_BASE* a = prechecked_cast<const ADP_BUILT_IN_MOS_BASE*>(adp());
  if(!a)untested0("no a");

  if (Umatch(x, "v ")) {
    return  _n[n_d].v0() - _n[n_s].v0();
  }else if (Umatch(x, "bar ")) {
    return  42.0;
  }else {
...
void ADP_BUILT_IN_MOS::tt_accept()
{
  ...
  int i;
  for (i=0; i < 3; i++) Hcloud[i] = Hcloudnew[i];
...
void ADP_BUILT_IN_MOS::tt_commit()
{
  ...
  double Ntotal = 60000;
  double Tox = 2.4e-9; // meters
  // ble D_0 = 5.64e-4;  // cm cm / s
  double kB = 8.61734315e-5; // boltz eV/K

  double A_0 = 4e7;
  double B_0 = 2.8e-7;
  double D_0 = 5.64e-8;  // m * m / s

  double T_ = 300; // kelvin
  double phiD = 0.45;
  double phi0 = 0.3;
  double phiAB = 0.45;
  double eina = 3.2e-10; // angstruem in meter


  double Dv = 1e-4;
  double A = ( A_0 / exp ( 1/ kB / T_ * ( 2.0 * phi0 - eina / Tox * bti_eff_voltage  ) ) );
  double B = ( B_0 / exp ( 1/ kB / T_ * ( phiD  ) ) );
  double D = ( D_0 / exp ( 1/ kB / T_ * ( phiD  ) ) );

  double leftdiffuse = sim->_dT0 * ( A * ( Ntotal - N ) - B * Hcloud[0] * N );
  Hcloudnew[0] = Hcloud[0] + leftdiffuse;
  Nnew = N - leftdiffuse;

  double diffuse1 = (Hcloud[1] - Hcloud[0])/ (Tox/2) * sim->_dT0;
  double diffuse2 = (Hcloud[2] - Hcloud[1])/ (Tox/2) * sim->_dT0;
  
  double diffuse3 = Hcloud[2] * sim->_dT0 * Dv;


  Hcloudnew[0] = Hcloud[0] + diffuse1   ;
  Hcloudnew[1] = Hcloud[1] - diffuse1 + diffuse2 ;
  Hcloudnew[2] = Hcloud[2] - diffuse2 - diffuse3 ;

  Hgonenew = Hgone - diffuse3;
...
double ADP_BUILT_IN_MOS::tr_probe_num(const std::string& x)const
{
  if( Umatch("bti ", x) ){
    ...
    return vthdelta_bti;
        ...
        //    return ADP_BUILT_IN_MOS::tr_probe_num(x); diode?
  }
...
void DEV_BUILT_IN_MOS::tr_save_amps(int trstep)
{
  ...
  hp_float_t tramps3= tramps0 + tramps1 + tramps2;
  //
  ...
  std::cerr << "DEV_BUILT_IN_MOS::tr_save_amps " << trstep << short_label() << \
